...
"""
Base Intelligence Agent for Dashboard AI Chat
Provides foundation for specialized dashboard agents
"""
...
class BaseIntelligenceAgent(ABC):
    ...
    def set_context(self, dashboard_data: Dict) -> None:
        ...
        Args:
            dashboard_data: Full dashboard data from ML models
        """
    ...
    def compress_context(self, full_context: Dict) -> Dict:
        ...
        Args:
            full_context: Full dashboard data
        ...
        # Ensure compressed context isn't too large
        import json
        compressed_str = json.dumps(compressed)
        if len(compressed_str) > 10000:  # Max 10KB
            # Further truncate
    ...
    def _extract_key_kpis(self, context: Dict) -> Dict:
        ...
        return dict(list(kpis.items())[:10])  # Limit to 10 KPIs
    ...
    def build_system_prompt(self, context: Optional[Dict] = None) -> str:
        ...
        Uses configuration from DocType if available.
    ...
    def execute(
        ...
        if not client.is_enabled():
            return {
                "success": False,
                "error": "AI Analytics is not enabled. Please configure your OpenRouter API key in Insights Settings.",
