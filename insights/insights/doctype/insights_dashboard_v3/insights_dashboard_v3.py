...
class InsightsDashboardv3(Document):
    ...
    def generate_dashboard_preview(self):
        with generate_preview_key() as key:
            ...
            if not preview:
                # Preview generation not available, skip silently
                return None
...
def get_page_preview(url: str, headers: dict | None = None) -> bytes:
    PREVIEW_GENERATOR_URL = frappe.conf.get("preview_generator_url")
    
    # If no preview generator URL is configured, return None silently
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            frappe.log_error(message=str(response.text), title="Failed to generate preview")
            return None
    except Exception as e:
        frappe.log_error(message=str(e), title="Preview generation error")
        return None
