<template>
  <div class="budget-variance-tab">
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        <h2 class="text-lg font-semibold text-gray-900">Budget Variance Analysis</h2>
    ...
    <div v-else class="space-y-6">
      <!-- Alerts Section -->
      <div v-if="budgetData.alerts && budgetData.alerts.length > 0" class="bg-red-50 border-l-4 border-red-400 p-4 rounded-r-lg">
        <div class="flex items-start">
          ...
          <div class="flex-1">
            ...
            <div class="mt-2 space-y-1">
              <div v-for="alert in (budgetData.alerts || []).slice(0, 3)" :key="alert.title" 
                   class="text-sm text-red-700">
                <span class="font-medium">{{ alert.title }}:</span> {{ alert.description }}
              </div>
            </div>
            <Button v-if="budgetData.alerts.length > 3" @click="showAllAlerts = !showAllAlerts" 
        ...
        <div class="bg-white rounded-lg border p-5">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold mt-1" :class="getVarianceColor(budgetData.summary?.variance_percentage || 0)">
          ...
          <div class="mt-3">
            <div class="flex items-center text-xs">
              ...
              <span class="ml-2 px-2 py-0.5 rounded-full text-xs font-medium"
                    :class="getStatusBadgeClass(budgetData.summary?.status)">
                {{ (budgetData.summary?.status || '').replace('_', ' ') }}
              </span>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg border p-5">
          ...
          <div class="mt-3">
            <div class="flex items-center text-xs">
              ...
              <span class="ml-2 capitalize">{{ budgetData.forecast_accuracy?.accuracy_trend || 'stable' }}</span>
            </div>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-lg border p-5">
          <h3 class="text-base font-semibold mb-4">Variance Trends</h3>
          <div v-if="budgetData.variance_trends?.monthly_variances" class="h-48">
            <div class="text-center py-8 text-gray-500">
              <BarChart class="w-8 h-8 mx-auto mb-2" />
              <p class="text-sm">Variance trend visualization</p>
              <p class="text-xs mt-1">{{ budgetData.variance_trends.monthly_variances.length }} months of data</p>
            </div>
          </div>
        ...
        <div class="bg-white rounded-lg border p-5">
          <h3 class="text-base font-semibold mb-4">Department Variance</h3>
          <div v-if="budgetData.departmental_analysis && budgetData.departmental_analysis.length > 0" class="space-y-3">
            <div v-for="dept in (budgetData.departmental_analysis || []).slice(0, 5)" :key="dept.department" 
        ...
        <div class="mt-6">
          ...
          <div v-if="activeDetailTab === 'departments'" class="space-y-4">
            <div v-if="budgetData.departmental_analysis && budgetData.departmental_analysis.length > 0">
              <div class="grid grid-cols-1 gap-4">
                <div v-for="dept in budgetData.departmental_analysis" :key="dept.department" 
                     class="border border-gray-200 rounded-lg p-4 hover:border-gray-300 transition-colors">
                  <div class="flex items-center justify-between mb-3">
                    <h4 class="font-medium text-gray-900">{{ dept.department }}</h4>
                    <span class="px-2 py-0.5 rounded-full text-xs font-medium"
                          :class="getStatusBadgeClass(dept.status)">{{ dept.status }}</span>
                  </div>
              ...
              <div class="overflow-x-auto shadow ring-1 ring-black ring-opacity-5 rounded-lg">
                <table class="min-w-full divide-y divide-gray-300">
                  ...
                  <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="account in (budgetData.account_analysis || []).slice(0, 10)" :key="account.account"
                      ...
                      <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">{{ formatCurrency(account.budget) }}</td>
                      <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">{{ formatCurrency(account.actual) }}</td>
          ...
          <div v-if="activeDetailTab === 'recommendations'" class="space-y-4">
            <div v-if="budgetData.recommendations && budgetData.recommendations.length > 0">
              <div class="grid grid-cols-1 gap-4">
                <div v-for="rec in budgetData.recommendations" :key="rec.title" 
                  ...
                  <div class="flex items-start justify-between">
                    <div class="flex-1">
                      <div class="flex items-center space-x-3 mb-2">
                        ...
                        <span class="px-2 py-0.5 rounded-full text-xs font-medium"
                              :class="getPriorityBadgeClass(rec.priority)">{{ rec.priority }}</span>
                        <span class="px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-700">{{ rec.category }}</span>
                      </div>
              ...
              <div class="bg-white rounded-lg border p-5">
                ...
                <div class="space-y-3">
                  ...
                  <div class="flex justify-between">
                    ...
                    <span class="font-medium">{{ budgetData.performance_metrics.budget_accuracy?.grade || 'N/A' }}</span>
                  </div>
                  <div class="flex justify-between">
              ...
              <div class="bg-white rounded-lg border p-5">
                ...
                <div class="space-y-3">
                  <div class="flex justify-between">
                    ...
                    <span class="font-medium">{{ Math.round(budgetData.performance_metrics.variance_control?.control_score || 0) }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-sm text-gray-600">Avg Variance</span>
                    <span class="font-medium">{{ formatPercentage(budgetData.performance_metrics.variance_control?.average_variance || 0) }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-sm text-gray-600">Control Level</span>
                    <span class="font-medium capitalize">{{ budgetData.performance_metrics.variance_control?.control_level || 'unknown' }}</span>
                  </div>
                </div>
              </div>
            </div>
...
const activeDetailTab = ref('departments')

const hasData = computed(() => {
  return budgetData.value && Object.keys(budgetData.value).length > 0
})
...
const getHighPriorityAlerts = computed(() => {
  return budgetData.value.alerts?.filter(alert => alert.severity === 'high') || []
})
...
const fetchData = async () => {
  ...
  try {
    ...
    if (result.message) {
      budgetData.value = result.message
    } else {
